...
This repo is the **sole home** of the native app code. The blueprint is
[`apple/ARCHITECTURE.md`](apple/ARCHITECTURE.md); the 10-minute Mac setup is
[`apple/README.md`](apple/README.md). Only each app's `Content/` + icons come from the
[FlyGACA-app](https://github.com/FlyGACA/FlyGACA-app) monorepo — see
[Content](#content-committed-snapshots) below.
...
| [`docs/`](docs/README.md) | Runbooks — the release path and the iOS runbook set, all owned here |
| [`apple/ARCHITECTURE.md`](apple/ARCHITECTURE.md) | The engineering blueprint (owned here) |
...
docs/                  RUNBOOK-ios-release + the RUNBOOK-ios-* set (all owned here);
...
school and reading paths where the pack has them). **This repo owns all its Swift
code and Xcode config**; only content comes from elsewhere. The **source of truth
for content is the [FlyGACA-app](https://github.com/FlyGACA/FlyGACA-app)
monorepo** — the regulatory corpus (`public/data/`) and the pack catalog
(`src/lib/prepCatalog.ts`). In this repo the `Content/` folders + icons are
committed snapshots. To refresh them:
...
That shells into the monorepo and runs its content + icon generators with
`--out apple/Apps`, writing straight into this repo. The monorepo's own `apple/`
mirror was **retired 2026-08**, so there is no `--all` mode and no Swift-code sync
anymore — this repo is the one home for the app code.
